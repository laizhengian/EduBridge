...
- **Fix:** the roster opens **all present by default** — the template *is* filled in. A tap moves a child through present → late → excused → absent (four states from day one; "late ≠ absent" is a live data bug in the old portal). Default-nothing is the old app's mistake; default-everyone-present means a well-behaved class is **two taps total** ("Everyone in?" → "Post"), and a normal class is one tap per exception. **Reasons are optional** — a quick-pick chip, a written line, or nothing; the post is never blocked waiting on a reason, and recurring reasons surface as "(again)" quick-picks from the term's history. A morning reminder arrives if attendance isn't taken by a set time (Deborah's ask). Saves autosave-as-draft, and the post shows an explicit server-confirmed "Saved" — because the portal's habit of losing entered marks (Nisha) is the trust-killer we are reversing. A **lateness tracker** (per student this term, with the recurring reason, ≥5 turning red) makes patterns visible without any searching.
