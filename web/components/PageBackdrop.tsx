/**
 * The school-notebook wallpaper: graph paper under a sparse scatter of
 * friendly emoji (pencils, flasks, newspapers, trophies…), covering the whole
 * viewport and staying quiet — felt, not read. Decorative only — aria-hidden,
 * no pointer events, behind the content; the paper boxes carry the app, this
 * fills the gaps between them. Colour comes from the theme tokens
 * (globals.css), so every accent palette and dark mode tints it for free.
 *
 ...
 * calendars. A hand-placed scatter down the full height (no two emojis on one
 * row, each with its own tilt and size) so it reads as doodled on, not
 * stamped.
 */
...
/** Hand-placed scatter down the full height — diagonal, no two on one row. */
const SPOTS = [
  { left: "7%", top: "10%" },
  { left: "72%", top: "7%" },
  { left: "28%", top: "40%" },
